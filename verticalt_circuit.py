class CircuitLoop:
  def __init__(self, ra, shared_bottom,shared_top,order):
    self.voltage = int(ra[0])
    self.top = int(ra[1])
    self.left = int(ra[2])
    self.bottom = int(ra[3])
    self.right = int(ra[4])
    self.shared_bottom = shared_bottom
    self.shared_top = shared_top

# this code can be improved
def calculate_weights(circuitLoop, row, order, prev_volts):
    i1=0
    i2=0
    i3=0
    i4=0
...
print("Enter comma separated: volts,top,left,bottom,right (ohms) for I1")

input_to_split = input()
cirlp1 = CircuitLoop(input_to_split.split(","), True, False,1)

print("Enter comma separated: volts,top ohms,left ohms,bottom ohms,right ohms for I2")

input_to_split = input()
cirlp2 = CircuitLoop(input_to_split.split(","), True, False,2)

print("Enter comma separated: volts,top ohms,left ohms,bottom ohms,right ohms for I3")

input_to_split = input()
cirlp3 = CircuitLoop(input_to_split.split(","), True, True,3)

print("Enter comma separated: volts,top ohms,left ohms,bottom ohms,right ohms for I4")

input_to_split = input()
cirlp4 = CircuitLoop(input_to_split.split(","), False, True,4)

row1 = calculate_weights(cirlp1,row1,1,0)
row2 = calculate_weights(cirlp2,row2,2,cirlp3.voltage)
row3 = calculate_weights(cirlp3,row3,3,cirlp3.voltage)
row4 = calculate_weights(cirlp4,row4,4,cirlp3.voltage)
